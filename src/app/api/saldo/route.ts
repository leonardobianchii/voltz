...
export const fetchSaldo = async (id_cliente: number) => {
  ...
  export const fetchHistoricoTransacoes = async (id_cliente: number) => {
    const response = await fetch(`http://127.0.0.1:5000/api/historico_transacoes/${id_cliente}`);
    if (!response.ok) {
  ...
  export const fetchHistoricoAbastecimento = async (id_cliente: number) => {
    const response = await fetch(`http://127.0.0.1:5000/api/historico_abastecimento/${id_cliente}`);
    if (!response.ok) {
  ...
  export const realizarRecarga = async (dados: {
    ...
    const response = await fetch(`http://127.0.0.1:5000/api/historico_transacoes/${dados.id_cliente}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(dados),
    });
